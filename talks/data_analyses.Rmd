...
group_by(sat_act,education, gender) %>%
  summarise(
    count = n(),
...
sat_act$id<-1:nrow(sat_act)
# Compute the analysis of variance
res.aov <- aov_ez("id","ACT",sat_act, between=c("education","gender"))
...
If we want to compare each level with the other, we can computed those with pairwise comparisons using `emmeans`.
``` {r two-way-ANOVA-post-hoc}
m1 <- emmeans(res.aov, ~ education_fac*gender_fac)
