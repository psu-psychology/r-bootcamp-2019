...
pacman::p_load(dplyr, tidyr, ggplot2, GGally, Rmisc, jtools, psych) #install (if needed) and load packages
```


# Why does a plotting package have grammar?

While it may sound strange, knowing the grammar of the `ggplot2` package is fundamental to being able to understand the core of how to use the package. Grammar is generally thought of as the syntactic rules of a language that can be combined with a variety of substantive material. Grammar or syntax provide the structure of how language can be expressed. In other words, the grammar of `ggplot2` is another way of saying the _rules_ of how to format a line (or lines) of code to give us a beautiful graph as the output. Importantly, other ways of creating graphics have syntax as well (including Excel and SPSS), but `ggplot` is going to be slightly different. 

The important intuition in plotting with the `ggplot2` package is the concept of _layering_. That is, `ggplot2` attempts to move users away from thinking about point and click interfaces that produce one single graphic. Instead, the hope is to give users the freedom to think about different aspects of the graphics they would like to create that can give a highly customizable graphics experience. In the references below, you'll see a link to the "R Graphics Cookbook", which should further one's intuition that `ggplot2` has the desirable property of allowing users to use more modular pieces of code to create beautiful plots exactly to the user's specification.
...
```{r label, out.width = "100%", fig.cap = "inspired by [this website](https://skillgaze.com/2017/10/31/understanding-different-visualization-layers-of-ggplot/).", echo=FALSE}
...
\
`plot my data please, and this time include ACT score on the x axis and verbal SAT scores on the y axis!! Also, let's visualize males and females differently.`
\
\
...
```{r}
gg_object_aes <- ggplot(data = sat.act, aes(x = ACT, y = SATV, color = gender, fill = gender))  #don't worry about the difference between color and fill for now
plot(gg_object_aes)
```
\
\

Now we have x and y axes specified that correspond to in the `aes()` argument. This is what many would consider the base of a ggplot object that we can now layer on top of. We still can't see our data and in order to do so we need to add a `geom_` layer to the plot.
...
The `facet_wrap()` and `facet_grid()` functions can split the data further into other panels on the basis of some variable that might be useful to visualize separately. For example, if we wanted different panels in our scatter plots to correspond to different levels of education we could add:
...
```{r}
gg_object_aes <- gg_object_aes + labs(x = "ACT scores", y = "SAT Verbal", title = "This is a pretty plot", subtitle =  "And also I'm done talking")
plot(gg_object_aes)
```
