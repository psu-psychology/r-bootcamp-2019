...
rg <- get_registration_data()
rg_clean <- clean_registration_data(rg)
```

We have `r dim(rg_clean)[1]` people registered so far.
...
# Table
pos_dept <- xtabs(formula = ~ position + dept, data = rg_clean)
knitr::kable(pos_dept)
```

```{r mosaic-plot, message=FALSE, warning=FALSE}
ggplot(data = rg_clean) +
