...
If we're going too slow for you, it's ok to work ahead or help a neighbor who's not so speedy.
...
Just for fun, here is some data about the languages Bootcamp registrants reported knowing something about:

```{r what-languages}
survey <- readr::read_csv(params$survey_fn)

survey %>%
  ggplot2::ggplot(.) +
  ggplot2::aes(x = lang_known, fill = lang_known) +
  ggplot2::geom_bar() +
...
Detailed instructions can be found at <http://psu-psychology.github.io/r-bootcamp-2019/before-the-bootcamp.html>.
...
Let's everyone log-on to RStudio Server Pro.
...
> Painful lesson #1: Computers are super-literal. They are anally literal. More anally literal than any human being you've ever had to deal with. You're not going to change them. Just deal.
...
> Complex programs are just long sequences of commands entered into something like the computer's hidden internal console and the computer's responses to those commands.
...
Imagine if human languages were like this:
...
Not surprisingly, there can be many similarities in the way computer languages are structured and human languages.

## Files and folders/directories

Before we talk about other things you can say to the computer, let's talk about where and how computers store data and instructions or commands.

Computers store data & instructions (programs, scripts etc.) in two types of memory:

  - **Long-term** (between work sessions) in *files*.
  - **Short-term** (during your work session) in *volatile memory*. It's volatile because these items will *not* be saved across sessions unless you do something to save them to a file.
  
We'll talk about files first.
  
Files are organised in directories or folders.
Directories or folders are a tree-like structure for organizing files.
Like trees, there can be multiple branches.
Directories/folders often have a hierarchy of branches.
...
Some computer systems hide the file extention part, but I most power users like to show it.
...
The 'tell me (get) the working directory' or `getwd()` command shows us this information.
...
I can change my working directory, but we're going to suggest you don't do that.
There are much easier and reproducible ways to do things in R right from the comfort of your working directory.
In any event, as you continue your jouRney with R, you'll want to practice a form of 'situational awareness': Where am I now (in my computer's file system).
...
Here's another useful command: `my_age <- 56`.

```{r}
my_age <- 56 # Rick's age
...
This command tells R to 'assign the name "my_age" to the number 56'.
...
But try train your fingers to type it the easy way.
...
But style matters.
It's like saying 'like' all the time. Like people will like understand you, but they'll wonder why you like say like all the time when you really need not. It's also a topic that could get you in a 'flame war'. 
**Avoid flame wars**.
...
If computers had invented math, I think that they would have separated making statements or commands from asking questions--for clarity.
...
So, `bigly`, `good_name`, `a_longer_good_name`, `Good_name1`, and even `thisIsCamelCaseNoUnderscores` are acceptable names to R, but not `!good`, `bad name` (has a space) or `1_very_bad_name` (starts with a number).
You'll note that some people use the hyphen or dash character in a file name, like `slow-r`.
That works because the file system (*nix, Mac OS, Windows) allows it.
To R, however, `slow-r` looks like you are subtracting `r` from `slow`.
So, R won't accept `slow-r` as a name for something; it thinks you are doing math.
...
We can talk about *numbers*: `75` or `4^2` (4 to the 2nd power), `3.14159`, or `1.5e-3` (0.0015) using scientific notation.
Notice that we type numbers 'in the nude'--without surrounding them with quotation marks.
...
R treats strings, numbers, and logical/Boolean values (`TRUE` and `FALSE`) as different beasts most of the time.
...
> If you want to embed a quoted segment within a string, you need to use double quotations on the outside and single ones on the inside.

```{r}
saying <- "A wise person said 'If at first you don't succeed'..."
saying
```
...
```
Installing a package creates a local copy of the software on your machine, but to use the package you have to load it into that volatile/active memory we talked about earlier.
We use the `library` command to load packages.

```
library(swirl)
```

> Notice that a little window pops up when you type at the command line giving you some suggestions about what might come next. These hints can be very helpful.

Then, start the program by typing `swirl()` and hitting the enter or return key.
...
The character string `"1"` is a completely *different* thing (different type) to the computer than the number `1`.
...
The main reason is pragmatic: It's relatively easy to make electronic circuits that are either "on" or "off".
If we decide that "on" means 1, and "off" means 0, then we can do computations with these states.
For example, we can decide 1 means `TRUE` and 0 means `FALSE` and do Boolean algebra, or we can say that `10` + `10` = `100` (2 + 2 = 4 in binary or base 2).

Or we can assign `110010` to the *character* `2`, `101011` to the *character* `+`, `111101` to the `=` character, and `110100` to the *character* `4`. This lets the computer use the sequence `110010` `101011` `110010` `111101` `110100` to represent `2+2=4`.

### (Optional aside) What sort of computations?

We've said that computers can do different kinds of operations (computations) on data.
But what does that really mean?
What sort of operations or computations can we do?
...
Note that the length here is not the total number of letters but the number of letter *groups*.
...
Take a few minutes to complete the `swirl` lessons related to these topics.
...
Notice that we tell R what shape the matrix should have by telling it the number of rows and columns using `nrow` and `ncol`.

When we give R a command like 'make a matrix' using `matrix()`, we have to supply it with information about the ingredients of that matrix.
Those ingredients are called *parameters* or *arguments*.
The `1:16` part tells R to use the numbers 1 to 16 as the components of the matrix.
The `nrow=4` part says to make a matrix with 4 rows.

> You **must** use an equal sign `=` to assign a value to a parameter or argument in a command. We explain why later.
...
By the way, if your data are in a matrix form, the `length()` command may not always work the way you want it to.
...
We'll talk more about the virtues of tidy data [^2] on Friday, but keeping your data tidy (observations in rows, variables in columns, longer than wider) is a best practice you'll want to adopt whenever possible.
...
(my_list_wnames <- list(first_name = 'donald', 
                        age = 73, address = '1600 Pennsylvania Ave'))
```
...
names(t_test_r_100)
```
So, it turns out that that nice print out of the t-test is made up of a list with different named components.
That will come in handy soon.

### Your turn

Let's get our hands dirty again by taking the relevant `swirl` lesson: `7: Matrices and Data Frames`.
...
We assign `<-` that vector of `TRUE/FALSE` values the human-readable name `only_males`.
...
```{r}
# M and F are mutually exclusive in this case, so F is just the opposite of M
...
Remember that the exclamation point `!` is the 'not' operator.
It's like taking the negative of a number.
...
We'll talk about how to use R Markdown to implement this technique later in the Bootcamp.
...
1.  Typing in a comment line so you know what's to come.
    ...
    - So something like `# R Bootcamp work 2019-08-22` might be a good name.
...
2. Saving the file.
    ...
    - My suggestion would be to have a structure like this:
        - `courses/2019_r_bootcamp/`
    - And then save the file as `2018_08_22_script.R` under that directory.

Later, I'm going to sharpen and modify this recommendation slightly by adding a step that takes advantage of RStudio's project management functionality.
