...
Two-way ANOVA test is used to evaluate simultaneously the effect of two grouping variables (A and B) on a response variable. First, let's see the count, mean and sd of each level.
...
print(res.aov)
```
We can conclude that education is statistically significant. ACT score is positively related with education. Also, there is the interaction effect between education and gender.

If we want to compare each level with the other, we can compute those with pairwise comparisons using `emmeans`.
...
<!-- TODO(Yiming?): This still needs to be debugged. -->
...
It is common in data analysis. For example, participants take more than one tasks or conditions. In longitudinal study, data is collected from each participants more than once. Assume there is different time point in this dataset. Let's check whether ACT scores vary by time point and education.
